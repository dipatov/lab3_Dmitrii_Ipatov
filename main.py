...
d1_ids, d2_ver, d_table = create_table(graph)
...
def cut_list(doc, lis):
    sorted(lis, key=lambda x: x ^ doc)
    lis = lis[:k]
    return lis

def add_to_list(lis, id, doc):
    lis += list(d_table[d2_ver[id]])
    lis = cut_list(doc, lis)
    return lis
...
if __name__ == '__main__':
    mat_next, mat_dist = next_way(graph)
    for i in range(1, 1001):
        add_doc(i)

    table_sizes = [len(d_table[i]) for i in range(1, 1001)]
    print('Минимальные размер таблицы:', min(table_sizes))
    print('Средний размер таблицы:', np.mean(table_sizes))
    print('Максимальный размер таблицы:', max(table_sizes))
    ...
    print('Минимальные размер пути:', min(path_sizes))
    print('Средний размер пути:', np.mean(path_sizes))
    print('Максимальный размер пути:', max(path_sizes))
